...
title(_window, {"Getted data", "Полученные данные"}, {_X, _Y-_H*0.45f, .frame=2, .height=Info, .horAli=GUI::Midle}),
...
forceText(_window, {"Last force: %.1f", "Последние усилие: %.1f"},
    {_X-(LanguagedText::getLanguage()==Language::Russian ? 0.055f : 0.035f), _Y+_H*0.29f, .horAli=GUI::Left}),
tempText(_window, {"Last temperature: %.1f", "Последняя температура: %.1f"},
    {_X-(LanguagedText::getLanguage()==Language::Russian ? 0.075f : 0.06f), _Y+_H*0.33f, .horAli=GUI::Left}),
counterText(_window, {"Packets getted: %d", "Пакетов получено: %d"}, {_X, _Y+_H*0.37f}),
