...
forceText(_window, _X-(LanguagedText::getLanguage()==Language::Russian ? 0.055f : 0.035f), _Y+_H*0.29,
    {"Last force: %.1f", "Последние усилие: %.1f"}, GUI::Aligment::Left),
tempText(_window, _X-(LanguagedText::getLanguage()==Language::Russian ? 0.075f : 0.06f), _Y+_H*0.33,
    {"Last temperature: %.1f", "Последняя температура: %.1f"}, GUI::Aligment::Left),
counterText(_window, _X, _Y+_H*0.37, {"Packets getted: %d", "Пакетов получено: %d"}),
notSavedText(_window, _X, _Y+_H*0.41, {"Not saved", "Не сохранено"}, 1),
